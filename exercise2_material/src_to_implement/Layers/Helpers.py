...
def padding(input_matrix, k):
    ...
    padding_top, padding_left = np.ceil(padding / 2).astype(int)
    padding_bottom, padding_right = padding - [padding_top, padding_left]
...
def kernel_reshape(k):
    output_k = np.zeros((k.shape[1], k.shape[0], *k.shape[2:]))
    for c in range(k.shape[1]):
        output_k[c] = k[:, c, :, :]
    return output_k


def get_conv_as_matmul(input_matrix, k):  # this fun return toeplitz_matrix and padded_input
    ...
    padding = np.array(k.shape) - 1
    padding_top, padding_left = np.ceil(padding / 2).astype(int)
    padding_bottom, padding_right = padding - [padding_top, padding_left]

    padded_matrix = np.pad(input_matrix, ((padding_top, padding_bottom), (padding_left, padding_right)))

    toeplitz_base = np.empty((input_matrix.shape[1], 0))
    toeplitz_final = np.empty((0, padded_matrix.size))

    for j in range(k_flipped.shape[0]):
        row = np.pad(k_flipped[j], ((0, padded_matrix.shape[1] - k_flipped.shape[1])))
        col = np.pad([k_flipped[j, 0]], ((0, input_matrix.shape[1] - 1)))
        toeplitz_base = np.c_[toeplitz_base, linalg.toeplitz(col, row)]

    for i in range(input_matrix.shape[0]):
        before = np.zeros((toeplitz_base.shape[0], i * padded_matrix.shape[1]))
        toeplitz = np.c_[before, toeplitz_base]
        after = np.zeros((toeplitz_base.shape[0], padded_matrix.size - toeplitz.shape[1]))
        toeplitz = np.c_[toeplitz, after]
        toeplitz_final = np.r_[toeplitz_final, toeplitz]
    return toeplitz_final, padded_matrix