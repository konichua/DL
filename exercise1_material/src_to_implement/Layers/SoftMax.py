...
class SoftMax(BaseLayer):
    ...
    def backward(self, error_tensor):
        ...
        tensor2 = np.einsum('ij,jk->ijk', self.input_tensor,
                            np.eye(self.input_tensor.shape[1], self.input_tensor.shape[1]))
        derivatives = tensor2 - tensor1

        return np.einsum('bi,bij->bj', error_tensor, derivatives)

        # return self.input_tensor * (error_tensor - np.sum(error_tensor * self.input_tensor))